...
typedef struct nodevalue {
	char * key; //no key on final nodevalue, n order will have n+1 pointers
	struct node * child; //e.g. if key is 6, child will be all less than 6
	struct nodevalue * next; // to next nodevalue in node.
} nodevalue;

typedef struct node {
	struct nodevalue * keys;
	int occupied;
} node;

/*
 *	table: the name of the table for which the B+ tree is created
 * 	key: describes the key used by the index (mostly for sizeof)
 *	volatileFlag: persistent or volatile
 */
int createBTree(char * table, char * key, int volatileFlag) {
	// Open table file (or get all attributes or csv or something?)
	// CODE HERE
	// get all the values for the specified key
	char ** keylist = malloc(10000);
	// keyList = getKey(key); some function that returns pointer to list

	// create initial node
	int maxOrder = BLOCKSIZE/(sizeof(nodevalue) + sizeof(char *) + sizeof(node *) + sizeof(nodevalue *));
	struct node initialNode;
	initialNode.occupied = 0; 
	initialNode.keys = malloc(BLOCKSIZE - sizeof(node));

	// create file

	int idx = 0;
	while (keylist[idx]) {
		bTreeInsert("btreename", key, keylist[idx]);
		idx++;
	}
...
int main() {
	printf("testing b plus tree indexing...\n");
	printf("%d\n", createBTree("test", "test", 0));
	return 0;
}
